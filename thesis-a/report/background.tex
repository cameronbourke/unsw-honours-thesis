...
With GCC for example, the \textit{-pg} flag will generate trace functions that automatically collect timing information, which by default, is written to a file called \textit{gmon.out}. The tool \textit{gprof} can then be used to view the data. Alternatively, user defined trace functions can be called instead via the GCC flag \textit{-finstrument-functions} \cite{ManGCC}. 
...
The RISC-V uses a standard naming convention to describe the ISAs supported in a given implementation. The ISA name format is RV[\#\#\#][abc...xyz] \cite{DocsRISCVISANaming}, where:

\begin{itemize}
    \item RV - Indicates a RISC-V architecture.
    \item \#\#\# - \{32, 64, 128\} indicate the width of the integer register file and the size of the user address space.
    \item abc..xyz - Indicates the set of extensions support by an implementation.
\end{itemize}

The RISC-V ISA specification defines three fixed performance counters (hpmcounter0 to hpmcounter2) which are dedicated for cycle count, real-time clock and instructions-retired respectively. It also supports up to 29 programmable performance counters (hpmcounter3-hpmcounter31) \cite{DocsRISVISAPerfCounters}.
